...
/**
 * Created by wangychu on 5/10/17.
 */
public class AddNewDataForPathsExplorePanel extends VerticalLayout {  private UserClientInfo currentUserClientInfo;
    private Window containerDialog;
    private String discoverSpaceName;
    private TextField idPart1TextField;
    private TextField idPart2TextField;
    private FindRelationInfoOfTwoAnalyzingDataPanel relatedFindRelationInfoOfTwoAnalyzingDataPanel;

    public AddNewDataForPathsExplorePanel(UserClientInfo userClientInfo){
        this.currentUserClientInfo=userClientInfo;
        setSpacing(true);
        setMargin(true);
        SecondarySectionTitle addNewDataSectionTitle=new SecondarySectionTitle("添加路径通过的数据节点");
        addComponent(addNewDataSectionTitle);

        HorizontalLayout inputFieldContainerLayout=new HorizontalLayout();

        Label dataIdfieldName=new Label(FontAwesome.KEY.getHtml()+" 数据节点 ID: <span style='font-style: italic;'># </span>", ContentMode.HTML);
        ...
        if(idPart1TextField.getValue()!=null&&idPart2TextField.getValue()!=null){
            if(idPart1TextField.isValid()&&idPart2TextField.isValid()){
                ...
                if(measurableValueVO==null){
                    Notification errorNotification = new Notification("数据校验错误","系统中不存在ID为 "+dataItemId+" 的数据节点", Notification.Type.ERROR_MESSAGE);
                    ...
                    if(getRelatedFindRelationInfoOfTwoAnalyzingDataPanel()!=null){
                        getRelatedFindRelationInfoOfTwoAnalyzingDataPanel().addPathExploreDatas(measurableValueVO.getId());
                        getContainerDialog().close();
                    }
                }
            }else{
                Notification errorNotification = new Notification("数据校验错误","请输入合法的数据节点 ID", Notification.Type.ERROR_MESSAGE);
                errorNotification.setPosition(Position.MIDDLE_CENTER);
                errorNotification.show(Page.getCurrent());
                errorNotification.setIcon(FontAwesome.WARNING);
            }
        }else{
            Notification errorNotification = new Notification("数据校验错误","请输入数据节点 ID", Notification.Type.ERROR_MESSAGE);
            errorNotification.setPosition(Position.MIDDLE_CENTER);
            errorNotification.show(Page.getCurrent());
            errorNotification.setIcon(FontAwesome.WARNING);
        }
    ...
    public FindRelationInfoOfTwoAnalyzingDataPanel getRelatedFindRelationInfoOfTwoAnalyzingDataPanel() {
        return relatedFindRelationInfoOfTwoAnalyzingDataPanel;
    }

    public void setRelatedFindRelationInfoOfTwoAnalyzingDataPanel(FindRelationInfoOfTwoAnalyzingDataPanel relatedFindRelationInfoOfTwoAnalyzingDataPanel) {
        this.relatedFindRelationInfoOfTwoAnalyzingDataPanel = relatedFindRelationInfoOfTwoAnalyzingDataPanel;
    }
}
